class ImageGallery extends HTMLElement {
    ...
    connectedCallback() {
        // Initialize the component once it is connected to the DOM
        this.initializeComponent();
    }

    initializeComponent() {
        // Call methods to initialize the component
        let slides = Array.from(this.children);
        ...
        if (this.getAttribute("caption") === "true") {
            this.createCaption();
            this.addCaption();
        }

        if (this.getAttribute("counter") != null) {
            this.counterType = this.getAttribute("counter");
            this.addCounter(this.counterType, slides);
        }

        if (this.getAttribute("dots") != null) {
            this.dots = this.getAttribute("dots");
            this.createDots();
            this.dotsContainer = this.shadowRoot.querySelector('#gallery-container > #dots-container');
        }

        // Hide navigation buttons if all items are visible or if the device is mobile
        if (this.isMobileDevice()) {
            this.productContainer.addEventListener('touchstart', (event) => {
                this.touchstartX = event.changedTouches[0].screenX;
            }, false);

            this.productContainer.addEventListener('touchend', (event) => {
                this.touchendX = event.changedTouches[0].screenX;
                this.handleSwipe();
            }, false);

            this.productContainer.addEventListener('touchmove', (event) => {
                event.stopPropagation();
            });
        }

        if (slides.length) {
            this.populateElements(slides);
        }
        if (this.dotsContainer) {
            this.initializeDots(slides, this.dotsContainer, this.dots);
        }
